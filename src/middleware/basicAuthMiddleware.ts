/**
 * This is my own patch taken from: https://github.com/eelkevdbos/elysia-basic-auth/pull/8 to properly skip CORS preflight checks
 */
...
type CredentialsMap = Record<BasicAuthCredentials["username"], BasicAuthCredentials>

export type BasicAuthCredentialOptions = { env: string } | { file: string } | BasicAuthCredentials[]
...
function newCredentialsMap(option: BasicAuthOptions["credentials"]): CredentialsMap {
 ...
 */
function strSafeEqual(actual: string, expected: string, encoding: BufferEncoding = "utf-8") {
  ...
  return timingSafeEqual(
    ...
    // @ts-expect-error this was from the source code
 ...
 */
function newCredentials(attrs?: Partial<BasicAuthCredentials>): BasicAuthCredentials {
 ...
 */
function checkCredentials(challenge: BasicAuthCredentials, credentialsMap: Record<string, BasicAuthCredentials>) {
  ...
  switch (typeof scope) {
    ...
    case "object":
      if (Array.isArray(scope)) {
        return (ctx: PreContext) => scope.some((s) => getPath(ctx.request).startsWith(s))
      }
      break
    default:
...
export function basicAuth(userOptions: Partial<BasicAuthOptions> = {}) {
  ...
  const skipRequest = (request: Request) => options.skipCorsPreflight && isCORSPreflightRequest(request)
